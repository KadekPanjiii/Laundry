                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-lg-6 mb-3">
                                        ...
                                        <input type="datetime-local" step="1" name="batas_waktu"
                                            class="form-control" placeholder="Batas Waktu" id="batas_waktu" require>
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        <label class="col-form-label">Member</label>
                                        <select class="form-control" name="member" require>
                                            <option value="" hidden>Pilih Member</option>
                                            @foreach ($data_member as $dm)
                                            <option value="{{ $dm->id }}">{{ ucwords ($dm->nama) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        <label class="col-form-label">Outlet</label>
                                        <select class="form-control" name="outlet" id="outlet" require>
                                            <option value="" hidden>Pilih Outlet</option>
                                            @foreach ($data_outlet as $do)
                                            <option value="{{ $do->id }}">{{ ucwords ($do->nama) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        ...
                                        <select class="form-control tombol-pilih" style="width: 100%;">
                                            ...
                                            <option value="{{ $dp->id }}" data-kode="{{ $dp->id }}" data-nama="{{ $dp->nama_paket }}" data-jenis="{{ $dp->jenis }}" data-harga="{{ $dp->harga }}">
                                                {{ ucwords(str_replace("_", " ", $dp->nama_paket)) ?? "-" }} - {{ ucwords(str_replace("_", " ", $dp->jenis)) ?? "-" }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
...
<script>
    ...
    function kembalikanData(pakets) {
        ...
        pakets.forEach(paket => {
            $(".tombol-pilih > option[data-kode=" + paket["kode"] + "]").click();
            $("#tableTransaksi .input-jumlah[data-kode=" + paket["kode"] + "]").val(paket["jumlah"]);
            $("#tableTransaksi .input-keterangan[data-kode=" + paket["kode"] + "]").val(paket["keterangan"]);
        });
    }
    ...
    $(".tombol-pilih").on("change", function () {
        ...
        let baris = table.row.add({
            "kode": kode,
            "nama": nama,
            "jumlah": "<input type='number' step='" + step + "' data-kode='" + kode + "' data-harga='" + harga + "' class='form-control form-control-sm input-jumlah' value='1' placeholder='Juml' required>",
            "harga": harga,
            "total": "<p data-kode='" + kode + "' class='total mb-0' style='color: black;'>" + harga + "</p>",
            "keterangan": "<input type='text' data-kode='" + kode + "' class='form-control form-control-sm input-keterangan' placeholder='Keterangan'>",
    ...
    $("#form-transaksi").on("submit", function(event){
        ...
        // Validasi harga bayar, sudah cukup/tidak
        ubahKembalian();
        ...
        $("#tableTransaksi tbody tr[data-kode]").each(function() {
            kode = $(this).find(".input-jumlah").attr("data-kode");
            jumlah = parseFloat($(this).find(".input-jumlah").val() || 0);
            jumlah = jumlah > 0 ? jumlah : 0;
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: "Apakah Anda yakin ingin menambahkan transaksi ini?",
            icon: 'warning',
